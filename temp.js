...
const CONFIG = {
    ...
    ARCHIVE_API_URL: '', // Tạm tắt do API đang bị chặn quyền 401 (domain ghn.vn)
...
async function loadAllData() {
    ...
    try {
        ...
        if (!proxySuccess) {
            try {
                ...
                [empRowsArrays, supRows, masterRowsGH, masterRowsPH, ctvRows] = await Promise.all([
                    ...
                    fetchSheetJSONP(CONFIG.SHEET_SUPPLIER_ID, CONFIG.SHEET_SUPPLIER_GID).catch(e => []),
                    fetchSheetJSONP(CONFIG.SHEET_MASTER_ID, CONFIG.SHEET_MASTER_GID).catch(e => []),
        ...
        if (empRows0.length === 0 && CONFIG.ARCHIVE_SHEET_ID) {
            ...
            try {
                ...
                if (archiveEmpRows.length > 0) {
                    ...
                    console.log(`✅ Archive ODO (FB3): ${archiveEmpRows.length} rows`);
                }
            } catch(archErr) {
        ...
        if (CONFIG.EXCLUDED_CTV_IDS) {
            CONFIG.EXCLUDED_CTV_IDS.forEach(id => ctvCodes.add(id));
        }
        ...
        allMasterRows.forEach(r => {
            ...
            const note = (r[9] || '').trim();
            const backupStr = ((r[10] || '') + ' ' + (r[7] || '')).trim().toUpperCase(); // Lấy cả cột K và cột H (đề phòng Proxy API bị giới hạn 10 cột A-J)
        ...
        if (container) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <small style="color:var(--text-muted);">Vui lòng thử lại sau hoặc liên hệ quản trị viên.</small><br><br><small style="color:red; font-family:Calibri, sans-serif;">Chi tiết lỗi: ${err.message}</small></p>
                </div>`;
        }
    }
}
...
function renderDailyTable(filtered) {
    ...
    filtered.forEach((e, i) => {
        ...
        html += `<tr>
            ...
            <td style="font-family:Calibri, sans-serif;white-space:nowrap;">${escapeHtml(e.hourEnd || '—')}</td>
...
function renderRanking(filtered, month, whFilter) {
    ...
    warehouseOrder.forEach(wh => {
        ...
        const finalEmpList = empList.filter(e => e && !(e.isSupport && e.myExpectedDays === 0) && !e.isResigned);
    ...
    try {
        ...
        if (typeof tripsData !== 'undefined' && Array.isArray(tripsData)) {
            tripsData.forEach(trip => {
                ...
                mappedNccPlates.forEach(nccP => {
                    ...
                    if (!ghnTripMap.has(mappedKey)) ghnTripMap.set(mappedKey, [trip.tripCode]);
        ...
        if (typeof employeeData !== 'undefined' && Array.isArray(employeeData)) {
            employeeData.forEach(e => {
                ...
                if (tripCode) {
                    ...
                    const codesToAdd = Array.isArray(tripCode) ? tripCode : [tripCode];
        ...
        allNccData.forEach(r => {
            if (!r.ghnTripCode) {
                ...
                let codes = ghnTripMap.get(matchKey); r.ghnTripCode = codes ? codes.join(' | ') : null;
            }
        });
        ...
        if (newRecordsToArchive.length > 0) {
            ...
            saveToArchive('ncc_trips_raw', newRecordsToArchive);
        }
...
function rematchNccTrips() {
    ...
    tripsData.forEach(trip => {
        ...
        if (trip.plate) {
            const normPlate = normalizeStr(trip.plate);
            const key = `${normPlate}_${trip.dateStr}`;
            if (!ghnTripMap.has(key)) ghnTripMap.set(key, [trip.tripCode]); else { let arr = ghnTripMap.get(key); if (!arr.includes(trip.tripCode)) arr.push(trip.tripCode); }
        }
        if (trip.driverId) {
            const driverKey = `${String(trip.driverId).trim()}_${trip.dateStr}`;
            if (!ghnDriverMap.has(driverKey)) ghnDriverMap.set(driverKey, [trip.tripCode]); else { let arr = ghnDriverMap.get(driverKey); if (!arr.includes(trip.tripCode)) arr.push(trip.tripCode); }
        }
    });
    ...
    nccTripData.forEach(r => {
        ...
        const normPlate = normalizeStr(r.plate);
        const matchKey = `${normPlate}_${r.dateStr}`;
        let codeArr = ghnTripMap.get(matchKey); let code = codeArr ? codeArr.join(' | ') : null;
        if (!code && r.driverId) {
            const driverKey = `${String(r.driverId).trim()}_${r.dateStr}`;
            let dCodeArr = ghnDriverMap.get(driverKey); code = dCodeArr ? dCodeArr.join(' | ') : null;
        }
        if (code) {
            r.ghnTripCode = code;
            matched++;
    ...
    filtered.forEach((r, i) => {
        ...
        html += `
            <tr>
                ...
                <td style="text-align:right; font-family:Calibri, sans-serif;">${formatMoneyCell(r.kmOverFee)}</td>
                <td style="font-family:Calibri, sans-serif; white-space:nowrap;">${(() => {
                ...
                <td style="text-align:right; font-family:Calibri, sans-serif;">${formatMoneyCell(r.otFee)}</td>
                <td style="text-align:right; font-family:Calibri, sans-serif;">${formatMoneyCell(r.monthlyRate)}</td>
...
async function loadLastmileData() {
    ...
    try {
        ...
        for (var i = 0; i < archivedData.length; i++) {
            ...
            if (CONFIG.EXCLUDED_CTV_IDS && (CONFIG.EXCLUDED_CTV_IDS.includes(r.driverId) || CONFIG.EXCLUDED_CTV_IDS.includes(r.createdById))) {
                continue;
            }
  ...
  function renderLastmile(preserveScroll = false) {
    ...
    for (var i = 0; i < lastmileData.length && filtered.length < 5000; i++) {
        ...
        if (filterStatus) {
            ...
            var isManual = typeof manualTripCodes !== 'undefined' && manualTripCodes.has(row.tripCode);
...
window.jumpToNccTrip = function(tripCode) {
    if (!tripCode) return;
