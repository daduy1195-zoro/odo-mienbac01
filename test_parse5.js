...
function parseNccTabData(rawData, nccName, ghnTripMap, sheetId, tabName, tabGid, sourceRowsArray) {
    ...
    if (headerRowIdx > -1) {
        ...
        for (let ri = scanStart; ri <= scanEnd; ri++) {
            ...
            for (let ci = 0; ci < hdr.length; ci++) {
                const h = hdr[ci];
                if (!h) console.log("CONTINUED"); continue;
    ...
    if (colKmStart === -1 || colKmEnd === -1) {
        ...
        for (let i = startRow; i < Math.min(startRow + 5, rawData.length); i++) {
            const row = rawData[i];
            if (!row) console.log("CONTINUED"); continue;
    ...
    if (!totalCostHasValue) {
        ...
        for (let tryCol = 22; tryCol <= 26; tryCol++) {
            if (tryCol === colTotalCost || usedCols.has(tryCol)) console.log("CONTINUED"); continue;
            for (let si = dataStartRow; si < Math.min(dataStartRow + 5, rawData.length); si++) {
                const row = rawData[si];
                if (!row) console.log("CONTINUED"); continue;
    ...
    for (let i = startRow; i < rawData.length; i++) {
        const row = rawData[i];
        if (!row || row.length < 5) console.log("CONTINUED"); continue;
        ...
        if (rowStr.includes('tổng chi phí') || (rowStr.includes('biển số') && (rowStr.includes('kho') || rowStr.includes('lộ trình')))) {
            ...
            if (colKmEnd > -1 && colKmDiff === -1) colKmDiff = colKmEnd + 1;
            console.log("CONTINUED"); continue; // Bỏ qua dòng header này
        }
        ...
        if (!dateStr) console.log("CONTINUED"); continue;
        ...
        if (!plate && !isOffByPlate) console.log("CONTINUED"); continue;
        ...
        if (isOffByPlate) {
            ...
            for (let j = i - 1; j >= Math.max(0, i - 31); j--) {
                if (!rawData[j] || !rawData[j][colDate > -1 ? colDate : 1] || !rawData[j][colDate > -1 ? colDate : 1].toString().match(/\d/)) console.log("CONTINUED"); continue;
            ...
            else console.log("CONTINUED"); continue; // Không thể tìm ra biển số thực sự
        }
        
        if (typeof formatPlate === "function") plate = formatPlate(plate);
        if (!dateStr.match(/\d{2}\/\d{2}\/\d{4}/)) console.log("CONTINUED"); continue;
        if (stt && isNaN(stt) && !stt.match(/^\d/)) console.log("CONTINUED"); continue;
        ...
        if (dateParts.length === 3) {
            ...
            if (rowDate > today) console.log("CONTINUED"); continue;
        }
        ...
        for (let ci = 7; ci <= 9; ci++) {
            ...
            if (v.includes(':') && !hourStart) { hourStart = v; console.log("CONTINUED"); continue; }
        ...
        if (!warehouse) console.log("CONTINUED"); continue;
