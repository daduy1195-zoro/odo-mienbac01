...
search = r"""          pivotData\[khoGroup\]\[bienSo\]\.chiPhi \+= Number\(String\(r\.totalCost \|\| '0'\)\.replace\(/\[\^\\d\\\.-\]/g, ''\)\);
...
replace = r"""          pivotData[khoGroup][bienSo].chiPhi += (typeof parseVietnameseNumber === 'function' ? parseVietnameseNumber(r.totalCost) : Number(String(r.totalCost || '0').replace(/[^\d\.-]/g, '')));
...
code = re.sub(search, replacer, code)
