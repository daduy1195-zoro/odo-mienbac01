...
function parseNccTabData(rawData, nccName, ghnTripMap, sheetId, tabName, tabGid, sourceRowsArray) {
    ...
    if (headerRowIdx > -1) {
        ...
        for (let ri = scanStart; ri <= scanEnd; ri++) {
            ...
            for (let ci = 0; ci < hdr.length; ci++) {
                const h = hdr[ci];
                if (!h) console.log("CONTINUED AT LINE", 56); continue;
    ...
    if (colKmStart === -1 || colKmEnd === -1) {
        ...
        for (let i = startRow; i < Math.min(startRow + 5, rawData.length); i++) {
            const row = rawData[i];
            if (!row) console.log("CONTINUED AT LINE", 91); continue;
    ...
    for (let si = dataStartRow; si < Math.min(dataStartRow + 5, rawData.length); si++) {
        ...
        if (!row) console.log("CONTINUED AT LINE", 151); continue;
    ...
    if (!totalCostHasValue) {
        ...
        for (let tryCol = 22; tryCol <= 26; tryCol++) {
            if (tryCol === colTotalCost || usedCols.has(tryCol)) console.log("CONTINUED AT LINE", 158); continue;
            for (let si = dataStartRow; si < Math.min(dataStartRow + 5, rawData.length); si++) {
                const row = rawData[si];
                if (!row) console.log("CONTINUED AT LINE", 161); continue;
    ...
    for (let i = startRow; i < rawData.length; i++) {
        const row = rawData[i];
        if (!row || row.length < 5) console.log("CONTINUED AT LINE", 185); continue;
        ...
        if (rowStr.includes('tổng chi phí') || (rowStr.includes('biển số') && (rowStr.includes('kho') || rowStr.includes('lộ trình')))) {
            ...
            for (let ci = 0; ci < hdr.length; ci++) {
                const h = hdr[ci];
                if (!h) console.log("CONTINUED AT LINE", 193); continue;
                if (h.includes('km vào') || h.includes('km đi') || h.includes('km bắt đầu')) colKmStart = ci;
                if (h.includes('km ra') || h.includes('km về') || h.includes('km kết thúc')) colKmEnd = ci;
                if (h.includes('km chạy') || h.includes('tổng km') || (h.includes('số km') && !h.includes('vào') && !h.includes('ra') && !h.includes('phát sinh') && !h.includes('/')) || h.includes('km chênh lệch') || h.includes('cự ly') || h.includes('quãng đường')) colKmDiff = ci;
                if (h === 'lộ trình' || h === 'tuyến đường' || h.includes('điểm giao') || h === 'tuyến') colRoute = ci;
            ...
            if (colKmEnd > -1 && colKmDiff === -1) colKmDiff = colKmEnd + 1;
            console.log("CONTINUED AT LINE", 217); continue; // Bỏ qua dòng header này
        }
        ...
        if (!dateStr) console.log("CONTINUED AT LINE", 227); continue;
        ...
        if (!plate && !isOffByPlate) console.log("CONTINUED AT LINE", 241); continue;
        ...
        if (isOffByPlate) {
            ...
            for (let j = i - 1; j >= Math.max(0, i - 31); j--) {
                if (!rawData[j] || !rawData[j][colDate > -1 ? colDate : 1] || !rawData[j][colDate > -1 ? colDate : 1].toString().match(/\d/)) console.log("CONTINUED AT LINE", 258); continue;
            ...
            if (!inferred) {
                for (let j = i + 1; j <= Math.min(rawData.length - 1, i + 31); j++) {
                    if (!rawData[j] || !rawData[j][colDate > -1 ? colDate : 1] || !rawData[j][colDate > -1 ? colDate : 1].toString().match(/\d/)) console.log("CONTINUED AT LINE", 269); continue;
                    let p = (rawData[j][colPlate > -1 ? colPlate : 2] || '').toString().trim().toUpperCase();
                    if (p && !p.includes('OFF') && !p.includes('NGHỈ') && !p.includes('NGHI') && p.length >= 6) {
                        inferred = p;
                        inferredWH = extractWHFromRow(rawData[j]);
            ...
            else console.log("CONTINUED AT LINE", 280); continue; // Không thể tìm ra biển số thực sự
        }
        
        if (typeof formatPlate === "function") plate = formatPlate(plate);
        if (!dateStr.match(/\d{2}\/\d{2}\/\d{4}/)) console.log("CONTINUED AT LINE", 284); continue;
        if (stt && isNaN(stt) && !stt.match(/^\d/)) console.log("CONTINUED AT LINE", 285); continue;
        ...
        if (dateParts.length === 3) {
            ...
            if (rowDate > today) console.log("CONTINUED AT LINE", 293); continue;
        }
        ...
        for (let ci = 7; ci <= 9; ci++) {
            ...
            if (v.includes(':') && !hourStart) { hourStart = v; console.log("CONTINUED AT LINE", 316); continue; }
        ...
        if (!warehouse) console.log("CONTINUED AT LINE", 337); continue;
